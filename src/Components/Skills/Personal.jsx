export const Personal = () => {
  return (
    <div className="skills1 pt-10 pb-10 mb-5 mt-7">
              <div className="flex justify-between w-[90%] text-[15px] ml-[5%] mr-[5%]">
                <p>Communication</p>
                <p>80%</p>
              </div>
              <div className="p-[5px] mt-[-5px] flex items-center w-[90%] ml-[5%] mr-[5%] border-[1px] border-white h-[20px] rounded-[6px]">
                <div className=" w-[80%] border-[3px] border-[skyblue] blue"></div>
              </div>
              <br />
              <div className="flex justify-between w-[90%] text-[15px] ml-[5%] mr-[5%]">
                <p>Cooking</p>
                <p>70%</p>
              </div>
              <div className="p-[5px] mt-[-5px] flex items-center w-[90%] ml-[5%] mr-[5%] border-[1px] border-white h-[20px] rounded-[6px]">
                <div className=" w-[70%] border-[3px] border-[skyblue] blue"></div>
              </div>
              <br />
              <div className="flex justify-between w-[90%] text-[15px] ml-[5%] mr-[5%]">
                <p>Playing Cricket</p>
                <p>85%</p>
              </div>
              <div className="p-[5px] mt-[-5px] flex items-center w-[90%] ml-[5%] mr-[5%] border-[1px] border-white h-[20px] rounded-[6px]">
                <div className=" w-[85%] border-[3px] border-[skyblue] blue"></div>
              </div>
              <br />
              <div className="flex justify-between w-[90%] text-[15px] ml-[5%] mr-[5%]">
                <p>Drawing</p>
                <p>65%</p>
              </div>
              <div className="p-[5px] mt-[-5px] flex items-center w-[90%] ml-[5%] mr-[5%] border-[1px] border-white h-[20px] rounded-[6px]">
                <div className=" w-[65%] border-[3px] border-[skyblue] blue"></div>
              </div>
              <br />
              <div className="flex justify-between w-[90%] text-[15px] ml-[5%] mr-[5%]">
                <p>Leadership</p>
                <p>90%</p>
              </div>
              <div className="p-[5px]  mt-[-5px] flex items-center w-[90%] ml-[5%] mr-[5%] border-[1px] border-white h-[20px] rounded-[6px]">
                <div className=" w-[90%] border-[3px] border-[skyblue] blue"></div>
              </div>
            </div>
  )
}
